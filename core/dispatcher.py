...
from .exceptions import BotAlreadyEnabled, NoSpaceToStartBot, NotFoundBot
...
async def set_bot_status(bot_service_id: int, status: Literal['online', 'offline']) -> None:
...
async def start_bot(bot_service_id: int, bot_token: str) -> None:
	if bot_service_id in bots:
		raise BotAlreadyEnabled()

	if len(bots) >= settings.max_bots_limit:
		raise NoSpaceToStartBot()
